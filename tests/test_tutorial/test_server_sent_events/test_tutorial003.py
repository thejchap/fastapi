...
def stream_logs(name: str):
    ...
    expect(data_lines[0], "first data line").to_equal(
        "data: 2025-01-01 INFO  Application started"
    )
    expect(data_lines[1], "second data line").to_equal(
        "data: 2025-01-01 DEBUG Connected to database"
    )
    expect(data_lines[2], "third data line").to_equal(
        "data: 2025-01-01 WARN  High memory usage detected"
    )
