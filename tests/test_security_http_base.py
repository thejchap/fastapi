...
@test("HTTPBase authenticates with valid credentials")
...
@test("HTTPBase tolerates extra whitespace in Authorization header")
def security_http_base_with_whitespaces():
    response = client.get("/users/me", headers={"Authorization": "Other  foobar "})
    expect(response.status_code, "status code").to_equal(200).fatal()
    expect(response.json(), "response body").to_equal(
        {"scheme": "Other", "credentials": "foobar"}
...
@test("OpenAPI schema for HTTPBase")
def openapi_schema():
